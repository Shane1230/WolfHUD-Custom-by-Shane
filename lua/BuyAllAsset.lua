...
elseif string.lower(RequiredScript) == "lib/managers/menu/missionbriefinggui" then
	...
	function AssetsItem:create_assets(...)
		...
		--add original buy all asset on bottom
		if not managers.preplanning:has_current_level_preplanning() and managers.menu:is_pc_controller() then
			...
			self.buy_all_button:set_top(250)
	...
	function AssetsItem:update_buy_all_btn(colors_only)
		if alive(self._buy_all_btn) then
			...
			if not colors_only then
				...
				self._buy_all_btn:set_top(10) --15
				if managers.menu:is_pc_controller() then
					self._buy_all_btn:set_left(15) --self._buy_all_btn:set_right(self._panel:w() - 5)
				else
	...
	Hooks:PostHook(MissionBriefingGui, "special_btn_pressed", "add_BuyAllAssets_key", function(self, ...)
		...
		local KEY_buyallasset = BLT.Keybinds:get_keybind("buy_all_asset_keybind"):Key()
		if Input:keyboard():pressed(Idstring(KEY_buyallasset)) then
			local is_preplanning = managers.preplanning:has_current_level_preplanning()
			local rebuy_reminder = managers.preplanning._rebuy_assets.reminder_active
			local can_rebuy = managers.preplanning:get_can_rebuy_assets()
