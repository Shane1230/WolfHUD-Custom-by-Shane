...
elseif string.lower(RequiredScript) == "lib/managers/hud/newhudstatsscreen" then
	...
	function HUDStatsScreen:_create_player_info()
		...
		if self._USING_CREW_LOADOUT then
			if alive(self._left) and not self._loadout_data then
				self._loadout_data = self._left:panel({
					name = "crew_loadout",
					w = self._left:w() - 20,
					h = HUDStatsScreen._LOADOUT_H + 380,
					visible = true
				})
		...
		if parent_panel then
			...
			for peer_id = 4, 1, -1 do
				...
				if panel and panel:enabled() and not panel:destroyed() then
					y = y - 98 --+ panel:h() + (parent_panel:h() * 0.08)
					panel:set_y(y)
				end
			end
			parent_panel:set_leftbottom(10, self._left:h() - (WolfHUD:getSetting({"TabStats", "ENABLED"}, true) and 10 or 40))
		end
	end
end