if string.lower(RequiredScript) == "lib/managers/hud/newhudstatsscreen" then
	...
	-- game original left stats (U199.3)
...
--난이도
		...
		--mission object
		if not self._USING_CREW_LOADOUT then
		...
		placer:add_bottom(loot_panel:fine_text({
			...
			text = managers.localization:to_upper_text("hud_stats_bags_secured_value"),
			font = medium_font,
			font_size = medium_font_size
		}), 12)
		placer:add_right(nil, 0)
...
--wolfhud left stats
	...
	function HUDStatsScreen:_create_stat_list(panel, ...)
		...
		if not (is_crime_spree or is_skirmish) then
			...
			{ text = managers.localization:to_upper_text("hud_stats_bags_secured_value"), font_size = self._tabstats_settings.FONT_SIZE or 18 },
			{ text = difficulty_text, font_size = self._tabstats_settings.FONT_SIZE or 18 }, nil, nil)
			placer:new_row()
