...
if string.lower(RequiredScript) == "lib/managers/hudmanagerpd2" then
	...
	function HUDListManager:_set_left_list_height_offset()
		local list = self:list("left_side_list")
		if list then
			list:move(list:panel():x(), HUDListManager.ListOptions.left_list_height_offset or 40, false) --+50 or 4, false
		end
	end
	...
	function HUDList.ItemBase:_animate_move(panel, x, y, instant, time_override)
		if not instant and self._move_speed > 0 then
			...
			local T = time_dride or math.max(math.abs(x - init_x) / move_speed, math.abs(y - init_y) / move_speed)
			local t = 0
	...
	HUDList.SpecialPickupItem.MAP = {
		...
		pardons = 					{ hudpickups = { 96, 96, 32, 32 }, 												priority = 1, category = "mission_pickups", ignore = not WolfHUD:getSetting({"HUDList", "RIGHT_LIST", "SHOW_PICKUP_CATEGORIES", "mission_pickups"}, true) 	},
		...
		blueprint = 				{ texture = "guis/dlcs/mex/textures/pd2/hud_pickups_blueprint", texture_rect = {  0,  0,32,32},		priority = 4, category = "mission_pickups", ignore = not WolfHUD:getSetting({"HUDList", "RIGHT_LIST", "SHOW_PICKUP_CATEGORIES", "mission_pickups"}, true) 	},
		weapon_stock = 				{ texture = "guis/dlcs/ranc/textures/pd2/equipment_stock",		texture_rect = {  0,  0,32,32},		priority = 5, category = "mission_pickups", ignore = not WolfHUD:getSetting({"HUDList", "RIGHT_LIST", "SHOW_PICKUP_CATEGORIES", "mission_pickups"}, true) 	},
		weapon_receiver = 			{ texture = "guis/dlcs/ranc/textures/pd2/equipment_receiver",	texture_rect = {  0,  0,32,32},		priority = 6, category = "mission_pickups", ignore = not WolfHUD:getSetting({"HUDList", "RIGHT_LIST", "SHOW_PICKUP_CATEGORIES", "mission_pickups"}, true) 	},
		weapon_barrel = 			{ texture = "guis/dlcs/ranc/textures/pd2/equipment_barrel",		texture_rect = {  0,  0,32,32},		priority = 7, category = "mission_pickups", ignore = not WolfHUD:getSetting({"HUDList", "RIGHT_LIST", "SHOW_PICKUP_CATEGORIES", "mission_pickups"}, true) 	},
